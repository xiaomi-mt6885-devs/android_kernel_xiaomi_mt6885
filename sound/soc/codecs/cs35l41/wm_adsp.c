...
#define WM_ADSP_FW_CALIB    10
#define WM_ADSP_FW_DIAG     11
#else
...
#ifdef CONFIG_SND_SOC_CS35L41_J10
	[WM_ADSP_FW_CALIB] =     "Calibration",
#else
	[WM_ADSP_FW_CALIB] =     "Diag Z",
#endif
	[WM_ADSP_FW_MISC] =     "Misc",
#ifdef CONFIG_SND_SOC_CS35L41_J10
	[WM_ADSP_FW_RCV_PROT] = "Protection RCV",
#endif
};
...
#ifdef CONFIG_SND_SOC_CS35L41_J10
	[WM_ADSP_FW_CALIB] =     { .file = "spk-cali" },
#else
	[WM_ADSP_FW_CALIB] =     { .file = "diag-z" },
#endif
	[WM_ADSP_FW_MISC] =     { .file = "misc" },
#ifdef CONFIG_SND_SOC_CS35L41_J10
	[WM_ADSP_FW_RCV_PROT] = { .file = "rcv-prot" },
#endif
};
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (dsp[e->shift_l].booted) {
		dev_err(codec->dev, "error happen, booted = %d\n", dsp[e->shift_l].booted);
		ret = -EBUSY;
	}
	else if (dsp[e->shift_l].compr)  {
		dev_err(codec->dev, "error happen, compr is NOT NULL\n");
		ret = -EBUSY;
	}
	else
...
static int wm_halo_load_state_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct wm_adsp *dsp = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.enumerated.item[0] = dsp->booted;

	return 0;
}

static int wm_halo_load_state_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
...
#endif
	SOC_SINGLE_EXT("DSP Firmware Loaded", SND_SOC_NOPM, 0, 1, 0,
			wm_halo_load_state_get, wm_halo_load_state_put),
};
...
static int wm_halo_setup_algs(struct wm_adsp *dsp)
{
	...
	size_t n_algs;
	int i, ret;
//	adsp_err(dsp, "wm_halo_setup_algs+++\n");
	mem = wm_adsp_find_region(dsp, WMFW_ADSP2_XM);
	if (WARN_ON(!mem))
		return -EINVAL;
...
static int wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	...
	if (dsp->tuning_has_prefix && component->name_prefix)
		snprintf(file, PAGE_SIZE, "%s-%s-dsp%d-%s.bin",
			 component->name_prefix, dsp->part,
...
static int wm_adsp_k_ctl_put(struct wm_adsp *dsp, const char *name, int value)
{
	...
	struct snd_ctl_elem_value ucontrol;
	struct snd_soc_card *card = dsp->codec->component.card;
...
static int wm_halo_apply_calibration(struct snd_soc_dapm_widget *w)
{
	...
	u32 status = 1;
	u32 chksum = status + dsp->cal_z;
	u32 calz_t =  cpu_to_be32(dsp->cal_z);
	u32 ambient_t = cpu_to_be32(dsp->ambient);
	...
	case WM_ADSP_FW_DIAG:
		dev_info(dsp->dev,
			"diagnostic AMBIENT <= %d\n", dsp->ambient);
		wm_adsp_write_ctl(dsp, "CAL_AMBIENT",
			&ambient_t, sizeof(ambient_t));
		break;
	case WM_ADSP_FW_SPK_PROT:
	case WM_ADSP_FW_RCV_PROT:
		dev_info(dsp->dev, "'%s' <= %d\n","CAL_R", dsp->cal_z);

		wm_adsp_write_ctl(dsp,
			"CAL_R", &calz_t, sizeof(calz_t));
		wm_adsp_write_ctl(dsp,
			"CAL_STATUS", &status, sizeof(status));
		wm_adsp_write_ctl(dsp,
			"CAL_CHECKSUM", &chksum,sizeof(chksum));

		wm_adsp_read_ctl(dsp,
			"CAL_R", &calz_t, sizeof(calz_t));

		dev_info(dsp->dev, "'%s' => %d \n",
			"CAL_R", be32_to_cpu(calz_t));
		break;
	default:
...
static struct wm_coeff_ctl *wm_adsp_find_host_buffer_ctrl(
						struct wm_adsp_compr_buf *buf)
{
	struct wm_adsp *dsp = buf->dsp;
	struct wm_coeff_ctl *ctl;
