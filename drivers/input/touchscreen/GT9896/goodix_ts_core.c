		...
		struct goodix_touch_data *touch_data)
{
	...
	if (goodix_ts_fod_event(core_data)) {
		...
		/*ts_info("event_status is 0x%x", core_data->event_status);*/
		fp_event_normal = 0;
	...
	if (fp_event_normal && !core_data->sleep_finger) {
		...
		ts_info("BTN_INFO press, fod_area %d,Report_0x152 DOWN_resume", core_data->fod_area);
	} else if (core_data->fod_pressed && !fp_event_normal) {
		if (unlikely(!core_data->fod_test)) {
			input_report_key(core_data->input_dev, BTN_INFO, 0);
			ts_info("BTN_INFO release, fod_area %d,Report_0x152 UP_resume", fod_up_area);
...
static void goodix_charger_in(struct goodix_ts_core *core_data)
{
	struct goodix_ts_device *ts_dev = core_data->ts_dev;
	...
	unsigned long event, void *data)
{
	...
	struct drm_notifier_data *fb_event = data;
	int blank;
...
static void gtp_set_cur_value_work(struct work_struct *work)
{
	...
	for (i = 0; i < Touch_Mode_NUM; i++) {
		...
		case Touch_Game_Mode:
			...
			goodix_game_mode &= 0xFC;
			goodix_game_mode |= temp_value;
			break;
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	pm_qos_add_request(&core_data->tp_qos_request, PM_QOS_CPU_DMA_LATENCY,
